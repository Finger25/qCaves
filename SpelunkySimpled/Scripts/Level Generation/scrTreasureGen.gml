    ...
    (collision_point(x-16, y-16, oSolid, 0, 0) or collision_point(x+16, y-16, oSolid, 0, 0) or collision_point(x-16, y-16, oBlock, 0, 0) or collision_point(x+16, y-16, oBlock, 0, 0)))
{
    ...
    else if (rand(1,10) == 1)
    {
        ...
        global.totalCratesGen += 1;//ADDED BY ME 
        //global.levelDeaths[global.customSeed - global.customSeedStart] += 1;//ADDED BY ME
    }
    else if (rand(1,15) == 1)
    {
        instance_create(x+8, y-8, oChest);
    }
    else if (not global.damsel and rand(1,8) == 1 and not collision_point(x+8, y-8, oWater, 0, 0))
    {

...
random_set_seed(1);
surely = random(10);
file_text_write_string(file, string(surely));
...
//ADDED BY ME
file_text_writeln(file);
file_text_write_string(file, "CUSTOM SEED STATS");
file_text_writeln(file);
file_text_write_string(file, "SEED:      ");
file_text_write_string(file, string(global.customSeed));
file_text_writeln(file);
file_text_write_string(file, "CRATES GENERATED:      ");
file_text_write_string(file, string(global.totalCratesGen));
file_text_writeln(file);
file_text_writeln(file);
file_text_writeln(file);
//this here seems to cause errors in oScreen real()
//game seems to run fine once you ignore errors
...
//HIJACKED BY ME
totalDeaths = 0;
for (i = 0; i < (global.customSeed - global.customSeedStart); i += 1)
{
    if (i+1 < 10) file_text_write_string(file, " " + string(global.customSeedStart + i) + "          | ");
    else file_text_write_string(file, " " + string(global.customSeedStart + i) + "         | ");
